...
exports.edit = (req, res, next) => {
    ...
    let { ID, MemberID, OperatorID = 1, Address, Connact, Telephone, TotalAmount, ReceiptAmount, PayStyle, DeliveryCompany = '', DeliveryFee = '', DeliverCode = '', DeliverReceiptFee = '', Remark = '', Date, Goods } = req.body;
    ...
    if (!MemberID || !Address || !Connact || !Telephone || !TotalAmount || !ReceiptAmount || !PayStyle || Goods.length == 0) {
        console.log("abaaa");
        res.status(422);
        return res.send({ code: 2, message: "参数不完整" });
    };

    const orderData = { ID, MemberID, OperatorID, Address, Connact, Telephone, TotalAmount, ReceiptAmount, PayStyle, DeliveryCompany, DeliveryFee, DeliverCode, DeliverReceiptFee, Remark, Date, Goods };
